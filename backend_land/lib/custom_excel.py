...
class CusStartParseExcel():
    ...
    def main(self):
        ...
        workbook = openpyxl.load_workbook('tester_custom2.xlsx')
        ...
        for idx, each in enumerate(MarkingDepartments):
            ...
            sheet.cell(row_line, 5).value = each.sheet5
            sheet.cell(row_line, 6).value = each.sheet6
            sheet.cell(row_line, 7).value = each.sheet8
            sheet.cell(row_line, 8).value = each.sheet9
            sheet.cell(row_line, 9).value = each.sheet11
            sheet.cell(row_line, 10).value = each.sheet12
            sheet.cell(row_line, 11).value = each.sheet13
            ...
            for idxx, eeach in enumerate(OtherShipDepartments):
                output1 = output1 + eeach.sheet9 + ', ' + eeach.sheet10+ ', ' + eeach.sheet11 + '\n'
            ...
            for idxx, eeach in enumerate(OwnershipDepartments):
                sheet.cell(row_line, 13).value = eeach.sheet5
                sheet.cell(row_line, 14).value = eeach.sheet6
                sheet.cell(row_line, 15).value = eeach.sheet7
                sheet.cell(row_line, 16).value = eeach.sheet9
                sheet.cell(row_line, 17).value = eeach.sheet10
                sheet.cell(row_line, 18).value = eeach.sheet11
                sheet.cell(row_line, 19).value = eeach.sheet13

                

                Buildings = models.OwnershipDepartmentHistory.objects.filter(ownershipdepartment=eeach)
                output1 = ''
                for idxxx, eeeach in enumerate(Buildings):
                    output1 = output1 + eeeach.sheet2 + ', ' + eeeach.sheet3+ '\n'
                sheet.cell(row_line, 20).value = output1
