...
class StartParseExcel():
    ...
    def main(self):
        ...
        sheet4_b = workbook['標示部-共有部分']
        sheet5_b = workbook['標示部-建物坐落地號']
        sheet6_b = workbook['標示部-附屬建物']

        workbook2 = openpyxl.Workbook()
        workbook2.create_sheet('標示部')
        workbook2.create_sheet('所有權部')
        workbook2.create_sheet('權利部')
        workbook2.create_sheet('標示部-共有部分')
        workbook2.create_sheet('標示部-建物坐落地號')
        workbook2.create_sheet('標示部-附屬建物')

        sheet = workbook2['標示部']
        sheet2 = workbook2['所有權部']
        sheet3 = workbook2['權利部']
        sheet4 = workbook2['標示部-共有部分']
        sheet5 = workbook2['標示部-建物坐落地號']
        sheet6 = workbook2['標示部-附屬建物']
        ...
        try:
            ...
            copy_sheet(sheet5_b, sheet5)
            copy_sheet(sheet6_b, sheet6)
        ...
        for idx, each in enumerate(MarkingDepartments):
            ...
            for idxx, eeach in enumerate(Buildings):
                sheet6.cell(Buildings_line, 1).value = each.sheet3+' / '+each.sheet4
                sheet6.cell(Buildings_line, 2).value = eeach.sheet1
                sheet6.cell(Buildings_line, 3).value = eeach.sheet2
                Buildings_line = Buildings_line+1

            for idxx, eeach in enumerate(Publics):
                sheet4.cell(Publics_line, 1).value = each.sheet3+' / '+each.sheet4
                sheet4.cell(Publics_line, 2).value = eeach.sheet1
                sheet4.cell(Publics_line, 3).value = eeach.sheet2
                sheet4.cell(Publics_line, 4).value = eeach.sheet3
                sheet4.cell(Publics_line, 5).value = eeach.sheet4
                sheet4.cell(Publics_line, 6).value = eeach.sheet5
                Publics_line = Publics_line +1 

            for idxx, eeach in enumerate(Locations):
                sheet5.cell(Location_line, 1).value = each.sheet3+' / '+each.sheet4
                sheet5.cell(Location_line, 2).value = eeach.sheet1
